...
# Inicialização do estado para o botão de ranking
if 'gerar_ranking' not in st.session_state:
    st.session_state.gerar_ranking = False

# Definindo df_tec_filt e col_t FORA do botão para evitar NameError
if not df_tec.empty:
...
if st.button("GERAR RANKING E ANÁLISES INFERENCIAIS"):
    st.session_state.gerar_ranking = True

# TODO O CÓDIGO ABAIXO SÓ SERÁ EXECUTADO APÓS O CLIQUE NO BOTÃO
if st.session_state.gerar_ranking:
    ...
    with c_sp2:
        st.markdown("<div class='info-card'><strong>Teste Qui-Quadrado Dinâmico</strong><br><span style='font-size: 0.85rem; color: #aaa;'>Analisa a dependência entre duas variáveis.</span>", unsafe_allow_html=True)
        ...
        if total_apas_reais < 10:
            st.warning(f"⚠️ N={total_apas_reais} insuficiente.")
            ...
            if not df_qui_clean.empty:
                ...
                if res_chi.get('valido', False):
                    st.write(f"Qui²: `{res_chi['chi2']:.2f}`")
                    st.write(f"P: `{res_chi['p_value']:.4f}`")

                    if res_chi['p_value'] < 0.05:
                        st.success("Existe Padrão.")
                    else:
                        st.info("Sem padrão.")
                else:
                    st.warning("Variância insuficiente.")
            else:
    ...
    st.markdown("---")
    st.markdown("<h4 style='color: #FFD700;'>📐 Modelagem Avançada</h4>", unsafe_allow_html=True)
