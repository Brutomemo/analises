...
""", unsafe_allow_html=True)
    ...
    with aba_geral:
        ...
        # ── CARREGA BACKGROUND UMA VEZ (OTIMIZAÇÃO) ──
        if "bg_serie_historica" not in st.session_state:
            try:
                with open("adesivo-caixa-de-Luz.webp", "rb") as f:
                    import base64
                    bg_b64 = base64.b64encode(f.read()).decode()
                    st.session_state["bg_serie_historica"] = bg_b64
            except FileNotFoundError:
                st.session_state["bg_serie_historica"] = None
        
        # ── APLICA CSS COM BACKGROUND ──
        if st.session_state["bg_serie_historica"]:
            bg_b64 = st.session_state["bg_serie_historica"]
            
            st.markdown(f"""
            <style>
            /* Background fixo na aba */
            ...
            /* Garante que conteúdo fica acima do overlay */
        ...
        st.markdown("<h5 style='color: #FFD700;'>📊 Série Histórica - Análises Globais</h5>", unsafe_allow_html=True)
        
        # ── RESTO DO CONTEÚDO DA ABA ──
        # (Seu código aqui mantém a mesma estrutura)
