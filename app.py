        ...
        if not lista_apas:
            ...
            if col_caus not in df_apa or col_neg not in df_apa:
                ...
                if not txt_caus or not txt_neg or len(txt_caus) < 20 or len(txt_neg) < 20:
                    ...
                    # ============================================================
                    # TAB 8: COM TRANSFORMER + TRADUÇÃO
                    # SUBSTITUA a seção do Transformer em TAB8_CORRIGIDA_FINAL.py
                    # ============================================================

                    # =========================================================
                    # SEÇÃO 2: ANÁLISE COM TRANSFORMER + TRADUÇÃO (Lazy Loading)
                    # =========================================================

                    st.markdown("---")
                    st.markdown("""
                    <div style='background: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; border-radius: 8px; margin-bottom: 20px;'>
                    <h5 style='color: #4CAF50; margin-top: 0;'>🤖 Análise Avançada com Transformer (Tradução Automática)</h5>
                    <p style='color: #aaa; font-size: 0.9rem; margin-bottom: 10px;'>
                    Esta análise traduz automaticamente para inglês e depois analisa sentimento com modelo treinado em inglês.
                    </p>
                    <ul style='color: #bbb; font-size: 0.9rem; line-height: 1.6; margin: 10px 0;'>
                    <li><strong>Tempo:</strong> Primeira execução ~20-30 segundos (traduz + carrega modelo)</li>
                    <li><strong>Vantagem:</strong> Melhor compreensão de português coloquial</li>
                    <li><strong>Como funciona:</strong> PT → EN → Análise de Sentimento</li>
                    <li><strong>Acurácia:</strong> ~80-85% em português (melhor que direto)</li>
                    <li><strong>Mostrada:</strong> Sentença original + tradução</li>
                    </ul>
                    ...
                    with col_center:
                        is_transformer = render_toggle_button(
                            label="🤖 Análise com Transformer + Tradução (20-30s primeira)",
                    ...
                    if is_transformer:
                        ...
                        if nlp_model is None:
                            ...
                            with st.spinner("Analisando fala do negociador (traduzindo + analisando)..."):
                                resultado_neg = analise.analise_sentimento_transformer_com_traducao(txt_neg, nlp_model)
                            ...
                            if resultado_neg:
                                ...
                                # Mostrar exemplos COM TRADUÇÃO
                                st.markdown("**Exemplos de Sentenças (com tradução):**")
                                ...
                                with col_pos:
                                    ...
                                    for r in [x for x in resultado_neg if x['label'] in ['POSITIVE', 'LABEL_2']][:3]:
                                        st.write(f"🇧🇷 {r['sentenca_pt']}")
                                        st.write(f"🇺🇸 _{r['sentenca_en']}_")
                                        st.write("")
                            ...
                            with st.spinner("Analisando fala do causador (traduzindo + analisando)..."):
                                resultado_caus = analise.analise_sentimento_transformer_com_traducao(txt_caus, nlp_model)
                            ...
                            if resultado_neg and resultado_caus:
                                ...
                                **Interpretação:**
                                ...
                                - Diferença: {abs(pct_pos_neg - pct_pos_caus):.0f}% → {"Alinhamento excelente" if abs(pct_pos_neg - pct_pos_caus) < 20 else "Alinhamento bom" if abs(pct_pos_neg - pct_pos_caus) < 30 else "Desencontro de tons"}
                                """)
